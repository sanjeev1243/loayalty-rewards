jQuery(function($) {
    ...
    var giftAvailablePoints = (typeof lrp_checkout_params !== 'undefined' && lrp_checkout_params.available_points)
        ? parseInt(lrp_checkout_params.available_points, 10)
        : 0;
...
$('#share_earn').on('click', function(e){
    ...
    $btn.prop('disabled', true).html('<i class="fas fa-share-alt" aria-hidden="true"></i><span>Sending...</span>');
    $.post(lrp_checkout_params.ajax_url, data, function(resp){
      $btn.prop('disabled', false).html('<i class="fas fa-share-alt" aria-hidden="true"></i><span>Share &amp; Earn</span>');
      if (!resp) {
...
$(document).on('click', '#copy_referral_code', function(e) {
    e.preventDefault();
    var $btn = $(this);
    var code = $btn.data('code') || $('#lrp_referral_code_text').text().trim();

    function showCopied() {
        $btn.addClass('is-copied').html('<i class="far fa-copy" aria-hidden="true"></i><span>Copied</span>');
        setTimeout(function() {
            $btn.removeClass('is-copied').html('<i class="far fa-copy" aria-hidden="true"></i><span>Copy</span>');
        }, 1600);
    }

    if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(code).then(showCopied, function() {
            $('#refer-message').text('Could not copy the code.').css('color','red').show();
        });
        return;
    }
   ...
   console.log('LRP params');
        if (e.type === 'keypress') {
			...
			console.log('LRP params',giftAvailablePoints);
            if (points > giftAvailablePoints) {
                cleanedValue = giftAvailablePoints.toString();
                points = giftAvailablePoints;
            }

            if (value !== cleanedValue) {
                $(this).val(cleanedValue);
                showNotice('Only whole numbers up to ' + giftAvailablePoints + ' points are allowed.', 'error');
            }
   ...
   $('#generate_gift_card').on('click', function () {
    ...
    $('#generate_gift_card').prop('disabled', true);
    $('#lrp-fullscreen-loader').fadeIn(200);
    ...
    .always(function () {
        ...
        $('#lrp-fullscreen-loader').fadeOut(200);
        $('#generate_gift_card').prop('disabled', false);
    });
	});
