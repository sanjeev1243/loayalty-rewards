<?php
/**
 * Module Name: LRP – Tier Updater (stand-alone, levels-only)
 * Calculates current/next tier level numbers and writes them into netscore_lty_cust_lty_pts_table.
 * Loaded by NetScore-loyalty-rewards.php; not a standalone WordPress plugin.
 */
...
final class LRP_Tier_Updater {
    ...
    public static function update( $user_id ) {
        ...
        $t_cust   = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
        $t_tiers  = $wpdb->prefix . 'netscore_lty_lty_tiers_table';
        $t_lvl    = $wpdb->prefix . 'netscore_lty_tier_lvl_pts_table';
        ...
        // Load active tiers with thresholds and level metadata (adjust column names if your schema differs)
    ...
    private static function write_levels_only( $user_id, $cur_lvl, $next_lvl ) {
        ...
        try {
            ...
            if ( $exists ) {
                ...
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- SET list is built from fixed whitelisted column names.
                ...
                // prepare + execute
                // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared -- Query is prepared above with collected values.
                $prepared = $wpdb->prepare( $sql, ...$values_for_prepare );
                $res = $wpdb->query( $prepared );

                if ( $res === false ) {
                    $err = $wpdb->last_error;
                    lrp_log( "[LRP] TierUpdater UPDATE failed for user {$user_id}: {$err}. Query: {$wpdb->last_query}" );
                ...
                // Customer row missing; do not insert here.
                $wpdb->query( 'ROLLBACK' );
                return false;
            }
        } catch ( Exception $e ) {
            $wpdb->query( 'ROLLBACK' );
            lrp_log( "[LRP] TierUpdater exception for user {$user_id}: " . $e->getMessage() );
            return false;
        }
    }
}
