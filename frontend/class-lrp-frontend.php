...
add_action( 'lrp_award_signup_points_if_eligible', array( $this, 'award_signup_points' ), 10, 1 );
add_action( 'woocommerce_created_customer', array( $this, 'award_referral_points' ), 20, 1 );
        ...
        add_action('woocommerce_cart_calculate_fees', function($cart) {
            ...
            if ( ! $this->is_feature_enabled( 'enable_points_redeem_on_checkout' ) ) {
                if ( WC()->session ) {
    ...
    private function get_feature_config() {
        ...
        global $wpdb;
        $defaults = $this->get_feature_defaults();
        ...
        $config = array_merge( $defaults, is_array( $row ) ? $row : [] );
        return $config;
    }
    ...
    private function get_feature_labels() {
        ...
        $defaults = $this->get_feature_defaults();
        $config = $this->get_feature_config();
        $labels = $defaults;
        foreach ( $defaults as $key => $fallback ) {
            if ( isset( $config[ $key ] ) && trim( (string) $config[ $key ] ) !== '' ) {
                $labels[ $key ] = sanitize_text_field( $config[ $key ] );
            }
        }
    ...
    private function get_feature_label( $key ) {
        $labels = $this->get_feature_labels();
        return $labels[ $key ] ?? '';
    }

    private function is_feature_enabled( $field ) {
        $config = $this->get_feature_config();

        if ( empty( $config['loyalty_eligible'] ) ) {
            return false;
        }
    ...
    private function render_feature_disabled_message( $message = 'This feature is disabled.' ) {
        echo '<div class="lrp-feature-disabled-message">' . esc_html( $message ) . '</div>';
    }
    ...
    public function add_menu_items( $items ) {
        ...
        foreach ( $items as $key => $label ) {
            ...
            if ( $key === 'dashboard' && ! $license_expired && $loyalty_eligible ) {
                $new_items['lrp_heading']            = $labels['my_account_tab_heading'];
                $new_items['loyalty-points-earned']  = $labels['loyalty_points_earned_label'];
                $new_items['redeem-points-history']  = $labels['redeem_history_label'];
                $new_items['refer-your-friend']      = $labels['refer_friend_label'];
                $new_items['generate-gift-card']     = $labels['gift_card_label'];
                $new_items['loyalty-tiers']          = $labels['tiers_label'];
                $new_items['update-profile']         = $labels['update_profile_label'];
            }
        }
        return $new_items;
}
    ...
    // Enqueue styles
    $lrp_styles_path = defined('LRP_PLUGIN_DIR') ? LRP_PLUGIN_DIR . 'assets/css/lrp-styles.css' : '';
    $lrp_styles_version = ( $lrp_styles_path && file_exists( $lrp_styles_path ) ) ? filemtime( $lrp_styles_path ) : '1.0.0';
    wp_enqueue_style('lrp-styles', LRP_PLUGIN_URL . 'assets/css/lrp-styles.css', array(), $lrp_styles_version);
    wp_enqueue_style('lrp-font-awesome', 'https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.15.4/css/all.min.css', array(), '5.15.4');

    $load_rewards_script = is_page('my-account/generate-gift-card') || is_account_page();
    if (
        is_checkout()
        && ! is_order_received_page()
        && is_user_logged_in()
        && ! LRP_Utils::is_site_license_expired()
        && $this->is_feature_enabled( 'enable_points_redeem_on_checkout' )
        && LRP_Utils::is_loyalty_eligible_customer()
    ) {
        $load_rewards_script = true;
    }

    // Enqueue rewards script only when the plugin renders rewards UI.
    if ($load_rewards_script) {
        wp_enqueue_script('lrp-checkout-script', LRP_PLUGIN_URL . 'assets/js/checkout.js', array('jquery'), '1.0.0', true);

        global $wpdb;
        $config_table = $wpdb->prefix . 'netscore_lty_lty_config_table';
        $config = null;
        ...
        if ($user_id) {
            ...
            $row = null;
            if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $t_cust_pts ) ) === $t_cust_pts ) {
                $row = $wpdb->get_row(
                    $wpdb->prepare("SELECT COALESCE(points_available,0) AS points_available FROM {$t_cust_pts} WHERE customer_id = %d LIMIT 1", $user_id),
                    ARRAY_A
                );
            }
...
public function display_loyalty_points() {
        if ( ! $this->is_feature_enabled( 'loyalty_eligible' ) ) {
            return;
        }
    if ( ! is_user_logged_in() && $this->is_feature_enabled( 'login_to_see_points' ) ) {
        return;
    }

    if ( is_user_logged_in() && ! LRP_Utils::is_loyalty_eligible_customer() ) {
        return;
    }
    ...
    $t_item  = $wpdb->prefix . 'netscore_lty_item_lty_pts_table';
    $t_cfg   = $wpdb->prefix . 'netscore_lty_lty_config_table';
    $t_tierL = $wpdb->prefix . 'netscore_lty_tier_lvl_pts_table';
    $t_tierP = $wpdb->prefix . 'netscore_lty_lty_tiers_table';
    $t_cust_pts = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';

    // --- Fetch product row (from item table only) ---
    ...
    // --- Global config points (each_point_value) ---
    ...
    if ( $available_points > 0 ) {
        // Fetch active tiers with thresholds (ordered asc)
    ...
    if ( ! $enable_collection ) {
        ...
        ?>
        <div class="lrp-product-points-banner">
            <span class="lrp-product-points-icon" aria-hidden="true"><i class="fas fa-gift"></i></span>
            <span class="lrp-product-points-divider" aria-hidden="true"></span>
            <span class="lrp-product-points-copy">
                <?php esc_html_e( 'Earn', 'NetScore Loyalty Rewards' ); ?>
                <strong><?php echo esc_html( sprintf( '%d Points', (int) $points ) ); ?></strong>
                <?php esc_html_e( 'with this purchase', 'NetScore Loyalty Rewards' ); ?>
            </span>
        </div>
        <?php
        return;
...
public function checkout_points() {
    ...
    global $wpdb;
    $user_id = get_current_user_id();
    // Fetch points_available from netscore_lty_cust_lty_pts_table (sum to be safe if multiple rows exist)
    $table_name = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    $available_points = 0.0;
...
$available_points = $available_points ?: 0;
    ...
    $threshold_table = $wpdb->prefix . 'netscore_lty_lty_config_table';
    $min_redemption_points = 1;
    if ( $wpdb->get_var( $wpdb->prepare( 'SHOW TABLES LIKE %s', $threshold_table ) ) === $threshold_table ) {
    ...
    // Calculate max_amount using netscore_lty_lty_config_table only
    $max_amount = ($each_point_value != 0) ? ($available_points / $each_point_value) * $loyalty_point_value : 0;

    // Calculate max redeemable points using netscore_lty_lty_config_table only
    ...
    // Calculate applied saving using netscore_lty_lty_config_table only
    ...
    wp_enqueue_script('lrp-checkout-script', LRP_PLUGIN_URL . 'assets/js/checkout.js', ['jquery'], '1.0.0', true);
    ...
    <div class="lrp-checkout-rewards">
        <h3><?php echo esc_html( $this->get_feature_label( 'product_redeem_label' ) ); ?></h3>
        <p>
...
public function apply_points_callback() {
    ...
    if ( ! $this->is_feature_enabled( 'enable_points_redeem_on_checkout' ) ) {
        wp_send_json_error( [ 'message' => 'Checkout feature is disabled.' ] );
        return;
    }

    $points = isset($_POST['points']) ? absint($_POST['points']) : 0;
    $user_id = get_current_user_id();
    // Fetch points_available from netscore_lty_cust_lty_pts_table (sum in case multiple rows)
    $table_name = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    $available_points = (float) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COALESCE(SUM(points_available), 0) FROM {$table_name} WHERE customer_id = %d",
...
$available_points = $available_points ?: 0;
    ...
    // Fetch minimum redemption points (account-level)
    $threshold_table = $wpdb->prefix . 'netscore_lty_lty_config_table';
    $min_redemption_points = (int) $wpdb->get_var( "SELECT minimum_redemption_points FROM {$threshold_table} LIMIT 1" );
    if ( $min_redemption_points <= 0 ) {
        $min_redemption_points = 1;
    ...
    public function remove_points_callback() {
        ...
        // Fetch updated points_available from netscore_lty_cust_lty_pts_table
        $user_id = get_current_user_id();
        $table_name = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
...
public function process_loyalty_points_redemption( $order_id ) {
    ...
    if ( $customer_type === 'netsuite' ) {
        ...
        // DO NOT:
        // - update netscore_lty_cust_lty_pts_table
        // - insert into netscore_lty_cust_lty_event_details_table
    ...
    $t_points_table  = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    ...
    try {
        ...
        $inserted = $wpdb->insert(
            ...
            $event_row,
            array(
                '%d','%s','%d','%s','%f','%f','%f','%d','%f','%s','%d','%s','%s','%d','%s','%s','%s'
            )
        );
        if ( $inserted === false ) {
    ...
    $eligible = (int) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COALESCE(is_eligible_for_loyalty_program, 0) FROM {$t_cust_pts} WHERE customer_id = %d LIMIT 1",
            $user_id
        )
    );
    if ( $eligible !== 1 ) {
    ...
    $event_row = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT event_name, is_active FROM {$t_events} WHERE event_id = %d LIMIT 1",
            $event_id
        )
    );
    ...
    // 2) Read signup points + config
    // Your schema: points_expiration_days (VARCHAR), not points_expiration_date
    $cfg = $wpdb->get_row(
        "SELECT customer_signup_points, each_point_value, loyalty_point_value, points_expiration_days 
         FROM {$t_config} 
         ORDER BY id DESC 
         LIMIT 1",
        ARRAY_A
    ...
    // if config row missing, fallback to sane default (matches table default)
    if ( $cfg ) {
        $points              = isset( $cfg['customer_signup_points'] ) ? floatval( $cfg['customer_signup_points'] ) : 0.0;
    ...
    $now_sql  = current_time( 'mysql' );
    $now_date = date( 'Y-m-d', $now_ts );

    $points_expiration_value = null;
    if ( $expiration_days_cfg > 0 ) {
        $points_expiration_value = date(
            'Y-m-d',
    ...
    try {
        ...
        $wpdb->query( 'ROLLBACK' );
        lrp_log( '[lrp] award_signup_points failure: ' . $e->getMessage() );
        return;
    }
}
...
public function award_review_points_on_transition( $new_status, $old_status, $comment ) {
    ...
    // Read points for product review from config (fallback to 0)
    $points = floatval( $wpdb->get_var( $wpdb->prepare( "SELECT product_review_points FROM {$t_config} LIMIT 1" ) ) );
    if ( $points <= 0 ) {
    ...
    // monetary equivalent (optional read from config)
    ...
    // event_name (optional lookup)
    $event_row = $wpdb->get_row( $wpdb->prepare( "SELECT event_name FROM {$t_events} WHERE event_id = %d LIMIT 1", $event_id ) );
    ...
    try {
        ...
        $wpdb->query( 'ROLLBACK' );
        lrp_log( '[lrp] award_review_points failure: ' . $e->getMessage() );
        return;
    }
}
...
public function award_purchase_points( $order_id ) {
    ...
    if ( $order->get_meta( '_lrp_points_awarded', true ) ) {
        return;
    }
    ...
    if ( $customer_type === 'netsuite' ) {
        // IMPORTANT: we do NOT set _lrp_points_awarded meta here,
        // so if in future you want a separate NetSuite-specific award flow,
        // it can still run.
        return;
    }
    ...
    $t_tierP     = $wpdb->prefix . 'netscore_lty_lty_tiers_table';
    $t_cust_pts  = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    $t_event_det = $wpdb->prefix . 'netscore_lty_cust_lty_event_details_table';
    $t_events    = $wpdb->prefix . 'netscore_lty_lty_events_table';
    ...
    foreach ( $required as $tbl ) {
        if ( ! $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $wpdb->esc_like( $tbl ) ) ) ) {
            lrp_log( "LRP ERROR: Required table missing: {$tbl}. Aborting awarding for order {$order_id}." );
            return;
        }
    }
    ...
    // Debug summary (will help troubleshooting)
    ...
    if ( $total_awarded <= 0 ) {
        ...
        $order->update_meta_data( '_lrp_points_awarded', 1 );
        $order->update_meta_data( '_lrp_points_awarded_amount', 0 );
        $order->save();
        return;
    }
    ...
    $event_exists_by_id = $wpdb->get_var( $wpdb->prepare( "SELECT event_id FROM {$t_events} WHERE event_id = %d LIMIT 1", $desired_event_id ) );
    $event_id_to_use = null;
    if ( ! $event_exists_by_id ) {
        $ev_by_name = $wpdb->get_var( $wpdb->prepare( "SELECT event_id FROM {$t_events} WHERE event_name = %s LIMIT 1", $desired_event_name ) );
        if ( $ev_by_name ) {
            $event_id_to_use = intval( $ev_by_name );
        } else {
            $ins = $wpdb->insert( $t_events, [ 'event_id' => $desired_event_id, 'NSID' => $desired_event_id, 'event_name' => $desired_event_name, 'is_active' => 1 ], [ '%d', '%d', '%s', '%d' ] );
            if ( $ins !== false ) {
                $event_id_to_use = $desired_event_id;
            }
        }
    } else {
    ...
    if ( $started === false ) {
        lrp_log( "LRP: Could not start DB transaction: " . $wpdb->last_error );
    }
    ...
    if ( $cust_row ) {
        ...
        if ( $upd === false ) {
            lrp_log( "LRP: Failed to update customer points for user {$user_id}: " . $wpdb->last_error );
            $wpdb->query( 'ROLLBACK' );
            return;
        }
    } else {
        ...
        if ( $ins === false ) {
            lrp_log( "LRP: Failed to insert customer points row for user {$user_id}: " . $wpdb->last_error );
            $wpdb->query( 'ROLLBACK' );
            return;
        }
    }
    ...
    if ( $ins_ev === false ) {
        lrp_log( "LRP: Failed to insert event_details for user {$user_id}, order {$order_id}: " . $wpdb->last_error );
        $wpdb->query( 'ROLLBACK' );
        return;
    }
    ...
    if ( $started !== false ) {
        ...
        if ( $committed === false ) {
            lrp_log( "LRP: Failed to commit DB transaction: " . $wpdb->last_error );
            $wpdb->query( 'ROLLBACK' );
            return;
        }
    }
    ...
    if ( $committed !== false ) {
        if ( class_exists( 'LRP_Tier_Updater' ) && method_exists( 'LRP_Tier_Updater', 'update' ) ) {
            try {
                ...
                lrp_log( "LRP: Exception when running LRP_Tier_Updater::update for user {$user_id}: " . $e->getMessage() );
            } catch ( Error $err ) {
                lrp_log( "LRP: Error when running LRP_Tier_Updater::update for user {$user_id}: " . $err->getMessage() );
            }
        } else {
            }
        }
    ...
    $order->update_meta_data( '_lrp_points_awarded', 1 );
    $order->update_meta_data( '_lrp_points_awarded_amount', $total_awarded );
    $order->save();
...
public function lrp_get_referral_code_for_user( $user_id ) {
    ...
    if ( empty( $code ) ) {
        ...
        $col_exists = $wpdb->get_var( $wpdb->prepare(
            ...
            $wpdb->prefix . 'netscore_lty_cust_lty_pts_table',
            'referral_code'
        ) );
        if ( $col_exists ) {
            $wpdb->update(
...
public function refer_friend() {
    if ( ! $this->is_feature_enabled( 'enable_refer_friend' ) ) {
        $this->render_feature_disabled_message();
        return;
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        var referCard = document.querySelector('.lrp-refer-card');
        if (referCard) {
            var accountContent = referCard.closest('.woocommerce-MyAccount-content');
            if (accountContent) {
                accountContent.classList.add('lrp-refer-account-content');
            }
        }
    });
    </script>
    <div class="lrp-refer-friend lrp-refer-card">
        <div class="lrp-refer-hero" aria-hidden="true">
            <i class="fas fa-gift"></i>
            <span class="lrp-refer-percent">%</span>
            <span class="lrp-refer-spark lrp-refer-spark-one"></span>
            <span class="lrp-refer-spark lrp-refer-spark-two"></span>
            <span class="lrp-refer-spark lrp-refer-spark-three"></span>
            <span class="lrp-refer-spark lrp-refer-spark-four"></span>
        </div>

        <h2><?php echo esc_html( $this->get_feature_label( 'refer_friend_label' ) ); ?></h2>
        <p class="lrp-refer-subtitle">On signup, you can get points &amp; they can get points too.</p>

        <div class="lrp-refer-divider"></div>

        <div class="lrp-refer-code-row">
            <span>Your Code:</span>
            <strong class="lrp-referral-code" id="lrp_referral_code_text"><?php echo esc_html( $referral_code ); ?></strong>
        </div>

        <div class="lrp-refer-input-row">
            <label class="lrp-refer-input" for="refer_email">
                <i class="far fa-envelope" aria-hidden="true"></i>
                <input type="email" id="refer_email" name="refer_email" placeholder="Enter email here...">
            </label>
            <!-- <button type="button" class="lrp-copy-referral" id="copy_referral_code" data-code="<?php echo esc_attr( $referral_code ); ?>">
                <i class="far fa-copy" aria-hidden="true"></i>
                <span>Copy</span>
            </button> -->
        </div>

        <button type="button" id="share_earn">
...
public function refer_friend_callback() {
    ...
    if ( ! $this->is_feature_enabled( 'enable_refer_friend' ) ) {
        wp_send_json_error( [ 'message' => 'This feature is disabled.' ] );
        return;
    }
    ...
    if ( ! $cust ) {
        ...
        if ( $inserted === false ) {
            lrp_log( '[lrp] refer_friend_callback: failed to create master row for referrer ' . $user_id . ' ; last_error=' . $wpdb->last_error );
    ...
    if ( empty( $referral_code ) ) {
        ...
        if ( $upd === false ) {
            lrp_log( '[lrp] refer_friend_callback: failed to store referral_code for user ' . $user_id . ' ; last_error=' . $wpdb->last_error );
    ...
    if ( ! $sent ) {
        lrp_log( '[lrp] refer_friend_callback: wp_mail failed for ' . $refer_email );
    ...
    if ( $ins === false ) {
        // log the DB error but don't treat mail as failed (mail already sent)
        lrp_log( '[lrp] refer_friend_callback: failed to insert invite ledger; last_error=' . $wpdb->last_error );
...
public function award_referral_points( $customer_id ) {
    ...
    if ( ! $this->is_feature_enabled( 'enable_referral_code_use_at_signup' ) ) {
        return;
    }
    ...
    if ( method_exists( 'LRP_Utils', 'get_admin_customer_type' ) ) {
        ...
        // If this site is a NetSuite customer, do NOT award referral points locally.
        if ( $customer_type === 'netsuite' ) {
            return;
        }
        // If 'loyalty' or '', continue with normal points logic below
    ...
    // fallback: also accept ?ref= query param (in case flow uses direct query)
    ...
    if ( $cfg && isset( $cfg['referral_points'] ) && floatval($cfg['referral_points']) > 0 ) {
        $referral_points = floatval( $cfg['referral_points'] );
    }
    ...
    $event_row = $wpdb->get_row( $wpdb->prepare( "SELECT event_name FROM {$t_events} WHERE event_id = %d LIMIT 1", 13 ) );
    ...
    try {
        ...
        $wpdb->query( 'ROLLBACK' );
        lrp_log( '[lrp] award_referral_points failure: ' . $e->getMessage() );
        return;
    }
}
    public function add_referral_code_field() {
        ...
        <p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
            <label for="referral_code"><?php esc_html_e('Referral Code (Optional)', 'NetScore Loyalty Rewards'); ?></label>
            <input type="text" class="woocommerce-Input woocommerce-Input--text input-text" name="referral_code" id="referral_code" value="<?php echo esc_attr(isset($_GET['ref']) ? sanitize_text_field( wp_unslash( $_GET['ref'] ) ) : ''); ?>">
        </p>
        <?php
    }
    public function save_referral_code_field($customer_id) {
        ...
        if (
            ...
            && function_exists( 'ns_lr_send_profile_to_netsuite' )
        ) {
            ns_lr_send_profile_to_netsuite( $customer_id );
        }
    }
public function loyalty_points_earned() {
    ...
    $available_percent = $total_earned > 0 ? min( 100, max( 0, ( $available_points / $total_earned ) * 100 ) ) : 0;
    $available_label   = number_format_i18n( $available_points, 2 );
    $earned_label      = number_format_i18n( $total_earned, 2 );
    $redeemed_label    = number_format_i18n( $total_redeemed, 2 );
    ?>
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        var rewardsDashboard = document.querySelector('.lrp-points-earned-dashboard');
        if (rewardsDashboard) {
            var accountContent = rewardsDashboard.closest('.woocommerce-MyAccount-content');
            if (accountContent) {
                accountContent.classList.add('lrp-rewards-account-content');
            }
        }
    });
    </script>
    <div class="lrp-account-dashboard lrp-points-earned-dashboard">
        <div class="lrp-account-header lrp-rewards-header">
            <div>
                <h2><?php echo esc_html( $this->get_feature_label( 'loyalty_points_earned_label' ) ); ?></h2>
                <p>Review the points credited to your loyalty account.</p>
            </div>
            <div class="lrp-rewards-header-art" aria-hidden="true">
                <i class="fas fa-gift"></i>
                <span class="lrp-header-card"><i class="fas fa-star"></i></span>
                <span class="lrp-header-coin lrp-header-coin-one"></span>
                <span class="lrp-header-coin lrp-header-coin-two"></span>
            </div>
        </div>

        <section class="lrp-rewards-hero">
            <div class="lrp-rewards-hero-main">
                <span>Available Points</span>
                <strong><?php echo esc_html( $available_label ); ?> <small>Points</small></strong>
                <p>Out of <?php echo esc_html( $earned_label ); ?> Points earned</p>
                <div class="lrp-reward-progress" aria-hidden="true">
                    <span style="width: <?php echo esc_attr( $available_percent ); ?>%;"></span>
                </div>
                <em><?php echo esc_html( number_format_i18n( $available_percent, 0 ) ); ?>% Available</em>
            </div>

            <div class="lrp-rewards-hero-side">
                <i class="fas fa-gift" aria-hidden="true"></i>
                <i class="fas fa-star" aria-hidden="true"></i>
            </div>
        </section>

        <div class="lrp-metric-grid">
            <div class="lrp-metric-card lrp-metric-earned">
                <span class="lrp-metric-icon"><i class="fas fa-gift" aria-hidden="true"></i></span>
                <span><small>Total Points Earned</small><strong><?php echo esc_html( $earned_label ); ?></strong><em>Points</em></span>
            </div>
            <div class="lrp-metric-card lrp-metric-redeemed">
                <span class="lrp-metric-icon"><i class="fas fa-shopping-bag" aria-hidden="true"></i></span>
                <span><small>Total Points Redeemed</small><strong><?php echo esc_html( $redeemed_label ); ?></strong><em>Points</em></span>
            </div>
            <div class="lrp-metric-card lrp-metric-available">
                <span class="lrp-metric-icon"><i class="fas fa-coins" aria-hidden="true"></i></span>
                <span><small>Available Points</small><strong><?php echo esc_html( $available_label ); ?></strong><em>Points</em></span>
            </div>
        </div>

        <section class="lrp-dashboard-card lrp-recent-activity">
            <div class="lrp-section-title">
                <h3>Recent Activity</h3>
                <a href="<?php echo esc_url( wc_get_account_endpoint_url( 'loyalty-points-earned' ) ); ?>">View All</a>
            </div>
            <div class="lrp-table-container">
                <table class="lrp-activity-table">
                    <thead>
                        <tr>
                            <th>Date</th>
                    ...
                    <tbody>
                        ...
                        if ( $rows ) {
                            foreach ( $rows as $row ) {
                                ...
                                echo '<tr>';
                                echo '<td><span class="lrp-activity-date"><i class="far fa-calendar-alt" aria-hidden="true"></i>' . esc_html( $display_date ) . '</span></td>';
                                echo '<td>' . esc_html( $activity ) . '</td>';
                                echo '<td>' . esc_html( $reference ) . '</td>';
                                echo '<td class="lrp-points-positive">+' . esc_html( $points_display ) . '</td>';
                                echo '</tr>';
                            }
                        } else {
                            echo '<tr><td colspan="4" class="lrp-empty-state">No points earned yet.</td></tr>';
                        }
                        ?>
                    </tbody>
                </table>
...
public function redeem_points_history() {
    if ( ! $this->is_feature_enabled( 'enable_redeem_history' ) ) {
        $this->render_feature_disabled_message();
        return;
    }
    ...
    $available_points = max( 0, $total_earned - $total_redeemed );
    $available_percent = $total_earned > 0 ? min( 100, max( 0, ( $available_points / $total_earned ) * 100 ) ) : 0;
    $available_label   = number_format_i18n( $available_points, 2 );
    $earned_label      = number_format_i18n( $total_earned, 2 );
    $redeemed_label    = number_format_i18n( $total_redeemed, 2 );
    ...
    // Render table
    ?>
    <div class="lrp-account-dashboard lrp-redeem-dashboard">
        <div class="lrp-account-header lrp-rewards-header">
            <div>
                <h2><?php echo esc_html( $this->get_feature_label( 'redeem_history_label' ) ); ?></h2>
                <p>Review the points you have redeemed from your loyalty account.</p>
            </div>
        </div>
        ...
        <!-- <section class="lrp-rewards-hero">
            ...
            <div class="lrp-rewards-hero-side">
                <div class="lrp-reward-ring" style="--available-percent: <?php echo esc_attr( $available_percent ); ?>%;" aria-label="<?php echo esc_attr( number_format_i18n( $available_percent, 0 ) ); ?>% Available">
                    <span>
                        <strong><?php echo esc_html( number_format_i18n( $available_percent, 0 ) ); ?>%</strong>
            ...
            <div class="lrp-rewards-gift" aria-hidden="true">
                <i class="fas fa-gift"></i>
                <span class="lrp-coin lrp-coin-one"></span>
                <span class="lrp-coin lrp-coin-two"></span>
            </div>
        </section> -->

        <!-- <div class="lrp-metric-grid">
            <div class="lrp-metric-card lrp-metric-earned">
                <span class="lrp-metric-icon"><i class="fas fa-arrow-up" aria-hidden="true"></i></span>
                <span><small>Total Points Earned</small><strong><?php echo esc_html( $earned_label ); ?></strong><em>Points</em></span>
            </div>
            <div class="lrp-metric-card lrp-metric-redeemed">
                <span class="lrp-metric-icon"><i class="fas fa-arrow-down" aria-hidden="true"></i></span>
                <span><small>Total Points Redeemed</small><strong><?php echo esc_html( $redeemed_label ); ?></strong><em>Points</em></span>
            </div>
            <div class="lrp-metric-card lrp-metric-available">
                <span class="lrp-metric-icon"><i class="fas fa-wallet" aria-hidden="true"></i></span>
                <span><small>Available Points</small><strong><?php echo esc_html( $available_label ); ?></strong><em>Points</em></span>
            </div>
        </div> -->
        ...
        <section class="lrp-dashboard-card lrp-recent-activity">
            ...
            <div class="lrp-table-container">
                <table class="lrp-activity-table lrp-redeem-table">
                    <thead>
                        <tr>
                            <th>Date</th>
                            <th>Activity Performed</th>
                    ...
                    <tbody>
                        ...
                        if ( $rows ) {
                            foreach ( $rows as $row ) {
                                ...
                                $display_date = $date_src ? date_i18n( 'd/m/Y', strtotime( $date_src ) ) : '-';
                                $activity     = ! empty( $row->event_name ) ? $row->event_name : '-';
                                $reference    = ! empty( $row->gift_code ) ? $row->gift_code : ( ! empty( $row->transaction_id ) ? $row->transaction_id : '-' );
                                ...
                                $amount_display  = function_exists( 'wc_price' ) ? '-' . wp_kses_post( wc_price( $amount ) ) : '-' . esc_html( number_format_i18n( $amount, 2 ) );

                                echo '<tr>';
                                echo '<td><span class="lrp-activity-date"><i class="far fa-calendar-alt" aria-hidden="true"></i>' . esc_html( $display_date ) . '</span></td>';
                                echo '<td>' . esc_html( $activity ) . '</td>';
                                echo '<td>' . esc_html( $reference ) . '</td>';
                                echo '<td class="lrp-points-negative">-' . esc_html( $points_display ) . '</td>';
                                echo '<td class="lrp-points-negative">' . wp_kses_post( $amount_display ) . '</td>';
                                echo '</tr>';
                            }
                        } else {
                            echo '<tr><td colspan="5" class="lrp-empty-state">No redemption history found.</td></tr>';
                        }
                        ?>
                    </tbody>
                </table>
...
public function generate_gift_card() {
    if ( ! $this->is_feature_enabled( 'enable_gift_certificate_generation' ) ) {
        $this->render_feature_disabled_message();
        return;
    }
    ...
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        var giftCard = document.querySelector('.lrp-gift-certificate');
        if (giftCard) {
            var accountContent = giftCard.closest('.woocommerce-MyAccount-content');
            if (accountContent) {
                accountContent.classList.add('lrp-gift-account-content');
            }
        }
    });
    </script>
    <div class="lrp-gift-certificate">
        <div class="lrp-gift-hero">
            <div class="lrp-gift-hero-icon" aria-hidden="true">
                <i class="fas fa-gift"></i>
            </div>
            <div class="lrp-gift-hero-copy">
                <h2><?php echo esc_html( $this->get_feature_label( 'gift_card_label' ) ); ?></h2>
                <p>Congratulations! You can turn your loyalty points into a gift card.</p>
            </div>
        </div>

        <div class="lrp-gift-panel">
            <div class="lrp-gift-stats" aria-label="Gift certificate balance">
                <div class="lrp-gift-stat">
                    <span class="lrp-gift-stat-icon" aria-hidden="true"><i class="fas fa-award"></i></span>
                    <span class="lrp-gift-stat-label">Points Available</span>
                    <strong><?php echo esc_html( number_format_i18n( $available_points, 0 ) ); ?></strong>
                </div>
                <div class="lrp-gift-stat">
                    <span class="lrp-gift-stat-icon" aria-hidden="true"><i class="far fa-credit-card"></i></span>
                    <span class="lrp-gift-stat-label">Max Amount</span>
                    <strong><?php echo wp_kses_post( wc_price( $max_amount ) ); ?></strong>
                </div>
            </div>

            <div class="lrp-gift-form">
                <label for="points_to_redeem">Points to redeem</label>
                <div class="lrp-gift-input">
                    <i class="fas fa-coins" aria-hidden="true"></i>
                    <input type="number" id="points_to_redeem" max="<?php echo esc_attr( $available_points ); ?>" min="1" placeholder="Enter points to redeem" <?php echo ( $available_points < $min_redemption_points ) ? 'disabled' : ''; ?> />
                </div>
                <div id="redeemAmountDisplay" aria-live="polite"></div>

                <label for="receiver_email">Receiver's Email</label>
                <div class="lrp-gift-input">
                    <i class="far fa-envelope" aria-hidden="true"></i>
                    <input type="email" id="receiver_email" placeholder="Enter receiver's email" <?php echo ( $available_points < $min_redemption_points ) ? 'disabled' : ''; ?> />
                </div>

                <div class="lrp-gift-note">
                    <i class="fas fa-info" aria-hidden="true"></i>
                    <span>Gift card will be sent to the email address provided.</span>
                </div>

                <button type="button" id="generate_gift_card" <?php echo ( $available_points >= $min_redemption_points ) ? '' : 'disabled'; ?>>
                    <i class="fas fa-gift" aria-hidden="true"></i>
                    <span><?php echo esc_html( $this->get_feature_label( 'gift_card_label' ) ); ?></span>
                    <i class="fas fa-arrow-right" aria-hidden="true"></i>
                </button>

                <?php if ( $available_points < $min_redemption_points ): ?>
                    <p class="lrp-gift-warning">You need at least <?php echo esc_html( $min_redemption_points ); ?> points in your account before you can redeem.</p>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
public function generate_gift_card_callback() {
    ...
    $expiry_date_sql  = date( 'Y-m-d H:i:s', $expiry_ts );
    $expiry_date_only = date( 'Y-m-d', $expiry_ts );

    // Event meta
    $event_id  = 2;
    $event_row = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT event_name FROM {$t_events} WHERE event_id = %d LIMIT 1",
            $event_id
        )
    );
    ...
    public function gift_card_history() {
        ...
        $users_table_sql = esc_sql( $wpdb->users );
        $results = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT gc.*, u.display_name, u.user_email
                 FROM {$table_name_sql} gc
                 LEFT JOIN {$users_table_sql} u ON gc.user_id = u.ID
                 WHERE gc.user_id = %d
        ...
        $total_items = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$table_name_sql} WHERE user_id = %d", $user_id ) );
        ?>
        <div class="lrp-table-container">
        ...
        if ($total_pages > 1) {
            ...
            for ( $i = 1; $i <= $total_pages; $i++ ) {
                ...
                echo '<li><a href="' . esc_url( add_query_arg( 'paged', $i, $base_url ) ) . '"' . ( $active ? ' class="' . esc_attr( $active ) . '"' : '' ) . '>' . esc_html( $i ) . '</a></li>';
            }
            if ( $current_page < $total_pages ) {
    ...
    $event_table = $wpdb->prefix . 'netscore_lty_cust_lty_event_details_table';
    $config_table = $wpdb->prefix . 'netscore_lty_lty_config_table';

    $pts_table_sql   = esc_sql( $pts_table );
    $event_table_sql = esc_sql( $event_table );
    $config_table_sql = esc_sql( $config_table );

    /* ---------- Load Config ---------- */
    $config = $wpdb->get_row(
        "SELECT birthday_points, anniversary_points FROM {$config_table_sql} LIMIT 1"
    );
    ...
    $today_mon  = (int) date( 'm', $today_ts );
    $today_date = date( 'Y-m-d', $today_ts );
    ...
    foreach ( $customers as $cust ) {
        ...
        if ( ! empty( $cust->birthdate ) ) {
            ...
            if ( date( 'Y-m-d', strtotime( $cust->birthdate ) ) === $today_date ) {
                continue;
            }
        ...
        if ( ! empty( $cust->anniversary_date ) ) {
            ...
            if ( date( 'Y-m-d', strtotime( $cust->anniversary_date ) ) === $today_date ) {
                continue;
            }
...
protected function award_profile_event_points( $user_id, $event_id, $event_name, $points, $note = '' ) {
    ...
    $t_cust_pts = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    $t_event_det = $wpdb->prefix . 'netscore_lty_cust_lty_event_details_table';
    $t_cust_pts_sql = esc_sql( $t_cust_pts );
    $t_event_det_sql = esc_sql( $t_event_det );

    // 1) Prevent double-award
    $already = (int) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COUNT(*) FROM {$t_event_det_sql}
             WHERE customer_id = %d
    ...
    $started = $wpdb->query( 'START TRANSACTION' );
    if ( $started === false ) {
        lrp_log( "LRP: Could not start transaction: " . $wpdb->last_error );
        // continue anyway; will rollback on failure
    }

    // 2) Upsert customer points row
    $cust_row = $wpdb->get_row( $wpdb->prepare(
        "SELECT id, customer_id, points_earned, points_available FROM {$t_cust_pts_sql} WHERE customer_id = %d LIMIT 1",
        $user_id
    ) );
    ...
    if ( $ins_ev === false ) {
        // Log failure and payload
        lrp_log( "LRP: Event insert failed for user {$user_id}: " . $wpdb->last_error );
        lrp_log( "LRP: event_insert_data: " . var_export( $event_insert_data, true ) );
        lrp_log( "LRP: last_query: " . $wpdb->last_query );
        ...
        if ( $ins_ev2 === false ) {
            lrp_log( "LRP: Fallback event insert also failed for user {$user_id}: " . $wpdb->last_error );
            if ( $started !== false ) { $wpdb->query( 'ROLLBACK' ); }
            return false;
        }
    }
    ...
    if ( class_exists( 'LRP_Tier_Updater' ) ) {
        try {
            ...
            lrp_log( "LRP: Tier update threw exception for user {$user_id}: " . $e->getMessage() );
        }
    }
}
...
public function update_profile() {
    if ( ! $this->is_feature_enabled( 'enable_profile_info' ) ) {
        $this->render_feature_disabled_message();
        return;
    }
    ...
    $config_table = $wpdb->prefix . 'netscore_lty_lty_config_table';
    $config_table_sql = esc_sql( $config_table );
    $config       = $wpdb->get_row( "SELECT birthday_points, anniversary_points FROM {$config_table_sql} LIMIT 1" );
    ...
    if ( $is_netsuite ) {
        ...
        $row       = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT birthdate, anniversary_date FROM {$pts_table_sql} WHERE customer_id = %d LIMIT 1",
                $user_id
            ),
            ARRAY_A
        );
    ...
    $update_profile_nonce = ! empty( $_POST['update_profile_nonce'] )
        ? sanitize_text_field( wp_unslash( $_POST['update_profile_nonce'] ) )
        : '';

    if (
        isset( $_POST['save_profile'] )
        && $update_profile_nonce
        && wp_verify_nonce( $update_profile_nonce, 'update_profile_action' )
    ) {

        // 🔧 FIX (Referral-only submit detection)
        $is_referral_only_submit =
    $is_netsuite
    && ! empty( $_POST['referral_code_by_friend'] )
    && empty( $_POST['birthday'] )
    && empty( $_POST['anniversary'] );
...
if ( $is_referral_only_submit ) {
    ...
    // ================== RENDER UI ==================
    ?>
    <div id="lrp-profile-display" class="lrp-update-profile-card">
        <div class="lrp-update-profile-header">
            <div class="lrp-update-profile-icon" aria-hidden="true">
                <i class="far fa-user"></i>
            </div>
            <div>
                <h2><?php echo esc_html( $this->get_feature_label( 'update_profile_label' ) ); ?></h2>
        ...
        <?php if ( $dob || $anniversary ): ?>
            ...
            <div class="lrp-profile-info lrp-update-profile-fields">
                <?php if ( $dob ): ?>
                    <div class="lrp-update-profile-row">
                        <label for="display-birthday">Date of Birth</label>
                        <input type="text"
                               id="display-birthday"
                               value="<?php echo esc_attr( date_i18n( get_option( 'date_format' ), strtotime( $dob ) ) ); ?>"
                               disabled="disabled">
                    </div>
                <?php endif; ?>
                ...
                <?php if ( $anniversary ): ?>
                    <div class="lrp-update-profile-row">
                        <label for="display-anniversary">Anniversary</label>
                        <input type="text"
                               id="display-anniversary"
                               value="<?php echo esc_attr( date_i18n( get_option( 'date_format' ), strtotime( $anniversary ) ) ); ?>"
                               disabled="disabled">
                    </div>
                <?php endif; ?>
                ...
                <?php if ( $tyre_type ): ?>
                    <div class="lrp-update-profile-row">
                        <label for="display-tyre-type">Tyre Type</label>
                        <input type="text"
                               id="display-tyre-type"
                               value="<?php echo esc_attr( ucfirst( $tyre_type ) ); ?>"
                               disabled="disabled">
                    </div>
                <?php endif; ?>
            </div>
            ...
            <?php if ( $is_netsuite ): ?>
                <div class="lrp-referral-code-section">
                    <?php if ( ! empty( $effective_referral_code ) ): ?>
                        <div class="lrp-update-profile-row">
                            <label for="display-referral-code">Referral Code</label>
                            <input type="text"
                                   id="display-referral-code"
                                   value="<?php echo esc_attr( $effective_referral_code ); ?>"
                                   disabled="disabled" />
                        </div>
                    <?php elseif ( empty( $signup_referral_code ) && empty( $profile_referral_code ) ): ?>
                        <form method="post" class="woocommerce-EditAccountForm edit-account lrp-referral-form lrp-update-profile-fields">
                            ...
                            <div class="lrp-update-profile-actions">
                                <button type="submit" name="save_profile" class="button">
        ...
        <?php else: ?>
            ...
            <form method="post" id="update-profile-form" class="woocommerce-EditAccountForm edit-account lrp-update-profile-fields">
                <?php wp_nonce_field( 'update_profile_action', 'update_profile_nonce' ); ?>
                ...
                <div class="lrp-update-profile-row">
                    <label for="anniversary">Anniversary <span>*</span></label>
                    <input type="date"
                           name="anniversary"
                           id="anniversary"
                ...
                <div class="lrp-update-profile-actions">
                    <button type="submit" id="save-profile-btn" name="save_profile" class="button">
                        <i class="far fa-save" aria-hidden="true"></i>
...
if ( $is_referral_only_submit ) {
    ...
    // ---------- Non-NetSuite: store in customer table + conditional award ----------
        ...
        $row          = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT id, birthdate, anniversary_date FROM {$pts_table_sql} WHERE customer_id = %d LIMIT 1",
                $user_id
            ),
            ARRAY_A
        );
...
public function loyalty_tiers() {
    if ( ! $this->is_feature_enabled( 'enable_tiers_info' ) ) {
        $this->render_feature_disabled_message();
        return;
    }
    ...
    $t_cust_pts        = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    $tiers_table       = $wpdb->prefix . 'netscore_lty_lty_tiers_table';
    $tier_levels_table = $wpdb->prefix . 'netscore_lty_tier_lvl_pts_table';
    $t_cust_pts_sql    = esc_sql( $t_cust_pts );
    $tiers_table_sql   = esc_sql( $tiers_table );
    $tier_levels_table_sql = esc_sql( $tier_levels_table );

    // Read authoritative available points from master table
    $available_points = 0;
    if ( $user_id ) {
        $available_points = $wpdb->get_var( $wpdb->prepare(
            "SELECT COALESCE(points_available,0) FROM {$t_cust_pts_sql} WHERE customer_id = %d LIMIT 1",
            $user_id
        ) );
    ...
    $tiers = $wpdb->get_results($wpdb->prepare("
        ...
        FROM {$tiers_table_sql} AS t
        LEFT JOIN {$tier_levels_table_sql} AS tl ON tl.tier_id = t.id
        WHERE t.status = %s
        ORDER BY COALESCE(tl.threshold, 0) ASC
    ", 'active'));
    ...
    $current_tier_name = $current_tier ? $current_tier->name : 'None';
    $next_tier_name    = $next_tier ? $next_tier->name : 'Top';
    $points_needed     = $next_tier ? max( 0, floatval( $next_tier->threshold ) - $available_points ) : 0;

    ?>
    <script>
    document.addEventListener('DOMContentLoaded', function() {
        var tierCard = document.querySelector('.lrp-tier-dashboard');
        if (tierCard) {
            var accountContent = tierCard.closest('.woocommerce-MyAccount-content');
            if (accountContent) {
                accountContent.classList.add('lrp-tier-account-content');
            }
        }
    });
    </script>
    <div class="lrp-loyalty-tiers lrp-tier-dashboard">
        <div class="lrp-tier-hero">
            <div class="lrp-tier-badge" aria-hidden="true">
                <i class="fas fa-crown"></i>
            </div>
            <div>
                <h2><?php echo esc_html( $this->get_feature_label( 'tiers_label' ) ); ?></h2>
                <p>
                    Current Tier: <strong><?php echo esc_html( $current_tier_name ); ?></strong>
                    <span aria-hidden="true">|</span>
                    Your Points: <strong><?php echo esc_html( number_format_i18n( $available_points, 0 ) ); ?></strong>
                </p>
            </div>
        </div>

        <div class="lrp-tier-body">
            <section class="lrp-tier-progress-panel">
                <h3>Progress to <?php echo esc_html( $next_tier ? $next_tier_name . ' Tier' : 'Top Tier' ); ?></h3>
                <div class="lrp-tier-progress-row">
                    <div class="lrp-tier-progress-track" aria-hidden="true">
                        <span style="width: <?php echo esc_attr( $progress_percentage ); ?>%;"></span>
                    </div>
                    <strong><?php echo esc_html( number_format_i18n( $progress_percentage, 0 ) ); ?>%</strong>
                </div>
                <?php if ( $next_tier ) : ?>
                    <p>Need <strong><?php echo esc_html( number_format_i18n( $points_needed, 0 ) ); ?></strong> more points to reach <?php echo esc_html( $next_tier_name ); ?>!</p>
                <?php else : ?>
                    <p>You are already in the highest available tier.</p>
                <?php endif; ?>
            </section>

            <section class="lrp-tier-available">
                <h3>Available Tiers</h3>
                <div class="lrp-tier-card-grid">
                    <?php
                    $display_tiers = [];

                    if ( $current_tier ) {
                        $display_tiers[] = $current_tier;
                    }

                    if ( $next_tier && ( ! $current_tier || intval( $next_tier->tier_id ) !== intval( $current_tier->tier_id ) ) ) {
                        $display_tiers[] = $next_tier;
                    }

                    foreach ( $display_tiers as $tier ) :
                        $is_current = $current_tier && intval( $tier->tier_id ) === intval( $current_tier->tier_id );
                        $tier_class = $is_current ? ' is-current' : '';
                        $name_lower = strtolower( (string) $tier->name );
                        if ( strpos( $name_lower, 'gold' ) !== false ) {
                            $tier_class .= ' is-gold';
                        } elseif ( strpos( $name_lower, 'silver' ) !== false ) {
                            $tier_class .= ' is-silver';
                        }
                        ?>
                        <article class="lrp-tier-card<?php echo esc_attr( $tier_class ); ?>">
                            <div class="lrp-tier-medal" aria-hidden="true">
                                <i class="fas fa-star"></i>
                            </div>
                            <div class="lrp-tier-card-copy">
                                <h4><?php echo esc_html( $tier->name ); ?> Tier</h4>
                                <p>Level <?php echo esc_html( intval( $tier->level ) ); ?>: <?php echo esc_html( number_format_i18n( floatval( $tier->threshold ), 0 ) ); ?> Points and above</p>
                                <div class="lrp-tier-card-rule"></div>
                                <p class="lrp-tier-multiplier"><i class="fas fa-chart-line" aria-hidden="true"></i><?php echo esc_html( number_format( floatval( $tier->points ), 2 ) ); ?>x Points Multiplier</p>
                            </div>
                        </article>
                    <?php endforeach; ?>
                </div>
            ...
            <?php if ( $next_tier ) : ?>
                <div class="lrp-tier-note">
                    <i class="fas fa-info" aria-hidden="true"></i>
                    <span>Earn more points to unlock <?php echo esc_html( $next_tier_name ); ?> Tier benefits and enjoy higher rewards!</span>
                </div>
            <?php endif; ?>
        </div>
    </div>
    ...
    public function get_current_tier($user_id) {
        ...
        $table_name = $wpdb->prefix . 'netscore_lty_lty_tiers_table';
        $table_name_sql = esc_sql( $table_name );
        $tiers = $wpdb->get_results( "SELECT name, threshold, points, level FROM {$table_name_sql} WHERE active = 1 ORDER BY threshold ASC" );
        if (!$tiers) {
            return null;
        }
        $current_tier = null;
...
public function display_social_share_buttons() {
    if ( ! $this->is_feature_enabled( 'product_sharing_through_email' ) ) {
        return;
    }
...
if ( ! is_user_logged_in() ) {
    ...
    $config = $wpdb->get_row( "SELECT email_share_points, facebook_share_points FROM {$table_sql} LIMIT 1" );
    $email_points = $config ? floatval($config->email_share_points) : 10.0; // Default to 10
    $fb_points = $config ? floatval($config->facebook_share_points) : 20.0; // Default to 20
...
public function share_social_callback() {
    ...
    $points = isset($_POST['points']) ? floatval($_POST['points']) : 0.0;
    ...
    $tier = $this->get_current_tier($user_id);
    if ($tier) {
        $points = $points * floatval($tier->points);
    }
