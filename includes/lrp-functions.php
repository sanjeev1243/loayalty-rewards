...
function lrp_refer_friend_callback() {
    ...
    $referral_points = floatval( get_option('lrp_referral_points', 50) );
    ...
    $html_message .= '<p>When you sign up with this code you will receive ' . esc_html( $referral_points ) . ' points.</p>';
    $html_message .= '<p>Thanks,<br>' . esc_html($site_name) . ' Team</p>';

    // Plain-text fallback
    $plain_message = sprintf(
        "Hello,\n\nYour friend has invited you to join.\n\nReferral Code: %s\n\nSign up: %s\n\nWhen you sign up with this code you will receive %s points.\n\nThanks,\n%s Team",
        $referral_code,
        $signup_url,
        $referral_points,
        $site_name
    ...
    if ( $sent ) {
        ...
        lrp_log( 'LRP: referral email failed to send to ' . $email . ' (referrer ' . $referrer_id . ').' );
...
function lrp_award_referral_points($new_user_id) {
    ...
    $referral_points = floatval(get_option('lrp_referral_points', 0));
    ...
    if (!$referral) {
        ...
        if ($wpdb->last_error) {
            lrp_log("lrp_award_referral_points: Failed to update referral status: {$wpdb->last_error}");
        }
    }
    ...
    if (!$ref_sent) {
        ...
        lrp_log("lrp_award_referral_points: Failed to send referrer notification to {$referrer->user_email}");
    }
    ...
    if (!$new_sent) {
        ...
        lrp_log("lrp_award_referral_points: Failed to send new user notification to {$new_user->user_email}");
    }
}
/**
...
function lrp_schedule_daily_tasks() {
    if (function_exists('lrp_is_netsuite_customer_type') && lrp_is_netsuite_customer_type()) {
        if (function_exists('wp_clear_scheduled_hook')) {
            wp_clear_scheduled_hook('lrp_check_birthdays');
            wp_clear_scheduled_hook('lrp_check_anniversaries');
        }
        return;
    }
    ...
    foreach ($users as $u) {
        ...
        if ($birthday && date('m-d') === date('m-d', strtotime($birthday))) {
            ...
            if (! get_user_meta($u->ID, $meta_year_key, true)) {
                $points = floatval(get_option('lrp_birthday_points', 25));
                if ($points > 0) {
...
function lrp_award_anniversary_points() {
    ...
    foreach ($users as $u) {
        ...
        if ($anniversary && date('m-d') === date('m-d', strtotime($anniversary))) {
            ...
            if (! get_user_meta($u->ID, $meta_year_key, true)) {
                $points = floatval(get_option('lrp_anniversary_points', 25));
                if ($points > 0) {
