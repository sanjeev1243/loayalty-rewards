...
class LRP_Customer_Events {
    ...
    // fetch page of events — include amount column, remove comments & event_id
...
echo '<h2 style="margin:0;">Events History of: ' . esc_html( $user_email ) . '</h2>';
    ...
    if ( $max_pages > 1 ) {
        ...
        if ( $page > 1 ) {
            echo '<a href="#" class="lrp-events-page button" data-customer-id="' . esc_attr( $customer_id ) . '" data-page="' . esc_attr( $page - 1 ) . '">&laquo; Prev</a> ';
        } else {
        ...
        for ( $p = $start; $p <= $end; $p++ ) {
            if ( $p == $page ) {
                ...
                echo '<a href="#" class="lrp-events-page button" data-customer-id="' . esc_attr( $customer_id ) . '" data-page="' . esc_attr( $p ) . '">' . esc_html( $p ) . '</a> ';
            }
        }

        if ( $page < $max_pages ) {
            echo ' <a href="#" class="lrp-events-page button" data-customer-id="' . esc_attr( $customer_id ) . '" data-page="' . esc_attr( $page + 1 ) . '">Next &raquo;</a>';
        } else {
