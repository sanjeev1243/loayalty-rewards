...
/**
 * NS Customer API
 * Single-route REST endpoints to create/get/update/delete netscore_lty_cust_lty_pts_table rows
 *
...
class NS_Customer_API {
    ...
    public function __construct() {
        global $wpdb;
        $this->table = $wpdb->prefix . 'netscore_lty_cust_lty_pts_table';
    ...
    protected function create_wp_user_by_email( $email ) {
        ...
        while ( username_exists( $candidate ) ) {
            $attempt++;
            $candidate = $base_login . rand( 1000, 9999 );
            if ( $attempt > 10 ) {
    ...
    public function get_handler( WP_REST_Request $request ) {
        ...
        $like_subscriber = '%"subscriber"%';

        // Main query: all customers/subscribers with LEFT JOIN on loyalty table
        $sql = "
            SELECT
                t.*,
                u.ID AS customer_id,
        ...
        // Optional: normalize rows that have no loyalty record
...
public function create_handler( WP_REST_Request $request ) {
    ...
    foreach ( $date_fields as $f ) {
        if ( array_key_exists( $f, $body ) ) {
            ...
            if ( $val === '' || is_null( $val ) ) {
                ...
                $ts           = strtotime( $val );
                $insert[ $f ] = $ts ? date( 'Y-m-d', $ts ) : null;
            }
            $format[] = '%s';
        }
    }
    ...
    if ( false === $inserted ) {
        // If insert failed due to race condition (duplicate created by another process),
        // try to fetch the existing row and return it.
        ...
        foreach ( $date_fields as $f ) {
            if ( array_key_exists( $f, $body ) ) {
                ...
                if ( $val === '' || is_null( $val ) ) {
                    ...
                    $ts = strtotime( $val );
                    $date_sql = $ts ? date( 'Y-m-d', $ts ) : null;
