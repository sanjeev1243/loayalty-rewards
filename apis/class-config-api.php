...
// Table and column identifiers are plugin-owned or whitelist-controlled before interpolation.
...
class LRP_Config_API {
    ...
    protected function get_args() {
        return [
            ...
            'customer_signup_points'    => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'product_review_points'     => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'referral_points'           => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'birthday_points'           => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'anniversary_points'        => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'each_point_value'          => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'loyalty_point_value'       => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'minimum_redemption_points' => [ 'required' => false, 'sanitize_callback' => 'absint' ],
            'email_share_points'        => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
            'facebook_share_points'     => [ 'required' => false, 'sanitize_callback' => [ $this, 'sanitize_decimal' ] ],
    ...
    public function sanitize_decimal( $value ) {
        return $value === null ? null : number_format( floatval( $value ), 2, '.', '' );
    }
    ...
    public function sanitize_events_array( $events ) {
        ...
        foreach ( $events as $event ) {
            ...
            if ( array_key_exists( 'event_id', $event ) ) {
                $clean_event['event_id'] = absint( $event['event_id'] );
            }
    ...
    private function get_events_table_name() {
        global $wpdb;
        $candidates = [
            $wpdb->prefix . 'netscore_lty_lty_events_table',
        ];
    ...
    private function sync_events_with_event_id( $incoming_events ) {
        global $wpdb;
        ...
        $this->ensure_events_event_id_column( $table );
        ...
        foreach ( $incoming_events as $event ) {
            ...
            if ( $event_id > 0 ) {
                ...
                $exists = $wpdb->get_row(
                    $wpdb->prepare( "SELECT * FROM {$table} WHERE NSID = %s LIMIT 1", $nsid ),
                    ARRAY_A
                );
            }
            ...
            if ( ! $exists ) {
                if ( ! $event_id ) {
                    $event_id = (int) $wpdb->get_var( "SELECT COALESCE(MAX(event_id), 0) + 1 FROM {$table}" );
                }
            ...
            if ( $event_id > 0 && (int) $exists['event_id'] !== $event_id ) {
    ...
    private function ensure_events_event_id_column( $table ) {
        global $wpdb;

        $column = $wpdb->get_var( $wpdb->prepare( "SHOW COLUMNS FROM {$table} LIKE %s", 'event_id' ) );
        if ( ! $column ) {
            $wpdb->query( "ALTER TABLE {$table} ADD event_id INT(10) UNSIGNED DEFAULT NULL AFTER id" );
        }

        $wpdb->query( "UPDATE {$table} SET event_id = id WHERE event_id IS NULL OR event_id = 0" );
        ...
        if ( ! $has_index ) {
            $wpdb->query( "ALTER TABLE {$table} ADD UNIQUE KEY uniq_event_id (event_id)" );
        }
    }
    ...
    private function return_full_payload( $config_row, $tiers_operation_result = null ) {
        ...
        if ( $events_table ) {
            $this->ensure_events_event_id_column( $events_table );
        }
        $events       = $events_table ? $wpdb->get_results( "SELECT * FROM {$events_table} ORDER BY event_id ASC, id ASC", ARRAY_A ) : [];
    ...
    public function create_or_update_config( WP_REST_Request $request ) {
        ...
        if ( $request->has_param( 'events' ) ) {
            ...
            $this->sync_events_with_event_id( $events );
        }
    ...
    public function update_config_by_id( WP_REST_Request $request ) {
        if ( $request->has_param( 'events' ) ) {
            $this->sync_events_with_event_id( $request->get_param( 'events' ) );
        }
    ...
    protected function get_format_for_params( $params ) {
        ...
        foreach ( $params as $key => $val ) {
            if ( in_array( $key, $this->decimal_fields, true ) ) {
                $formats[] = '%f';
