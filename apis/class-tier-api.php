...
class LRP_Tiers_API_Slim {
    ...
    protected function create_tier_single( WP_REST_Request $request ) {
        global $wpdb;
        $t_tiers    = $wpdb->prefix . 'netscore_lty_lty_tiers_table';
        $t_tier_lvl = $wpdb->prefix . 'netscore_lty_tier_lvl_pts_table';
        ...
        if ( false === $inserted ) {
            lrp_log( '[lrp] create_tier insert failed: ' . $wpdb->last_error );
        ...
        if ( isset( $data['threshold'] ) || isset( $data['points_for_currency'] ) || isset( $data['level'] ) ) {
            ...
            if ( false === $ins ) {
                lrp_log( '[lrp] Failed to insert level for tier ' . $id . ': ' . $wpdb->last_error );
            }
        }
    ...
    protected function update_tier_single( WP_REST_Request $request ) {
        ...
        if ( ! empty( $tier_data ) ) {
            ...
            if ( false === $updated ) {
                lrp_log( '[lrp] Tier update failed: ' . $wpdb->last_error );
        ...
        if ( $level_id ) {
            ...
            if ( ! empty( $lvl_data ) ) {
                ...
                if ( false === $upd ) {
                    lrp_log( '[lrp] Level update by id failed: ' . $wpdb->last_error );
        ...
        // 2) level provided -> either rename (if level_new present) or update existing fields
            ...
            if ( ! is_null( $level_target ) ) {
                ...
                if ( false === $upd ) {
                    lrp_log( '[lrp] Level rename failed: ' . $wpdb->last_error );
                ...
                if ( ! empty( $lvl_data ) ) {
                    ...
                    if ( false === $upd2 ) {
                        lrp_log( '[lrp] Level update after rename failed: ' . $wpdb->last_error );
                        return new WP_REST_Response( [ 'message' => 'Level update failed' ], 500 );
                    }
                }
            ...
            // Update without rename
                ...
                if ( ! empty( $lvl_data ) ) {
                    ...
                    if ( false === $upd ) {
                        lrp_log( '[lrp] Level update failed: ' . $wpdb->last_error );
                        return new WP_REST_Response( [ 'message' => 'Level update failed' ], 500 );
                    }
                }
            }
        ...
        // 3) No specific level, but level fields present => bulk update ALL levels of this tier
            ...
            if ( ! empty( $lvl_data ) ) {
                ...
                if ( false === $upd ) {
                    lrp_log( '[lrp] Bulk level update failed: ' . $wpdb->last_error );
                    return new WP_REST_Response( [ 'message' => 'Level update failed' ], 500 );
                }
            }
        }
    ...
    protected function delete_tier_single( WP_REST_Request $request ) {
        ...
        if ( false === $deleted ) {
            lrp_log( '[lrp] Tier delete failed: ' . $wpdb->last_error );
